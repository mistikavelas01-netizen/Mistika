...
function validateEnv(): { ok: boolean; error?: string } {
  if (!process.env.RESEND_KEY) {
    return { ok: false, error: "RESEND_KEY no está configurado" };
  }

  if (!process.env.RESEND_FROM_EMAIL) {
    return { ok: false, error: "RESEND_FROM_EMAIL no está configurado" };
  }

  if (!isValidEmail(process.env.RESEND_FROM_EMAIL)) {
    return { ok: false, error: "RESEND_FROM_EMAIL no es un correo válido" };
  }
...
export async function sendMail({
  ...
  try {
    ...
    if (!isValidEmail(to)) {
      ...
      return { ok: false, error: "Correo de destinatario inválido" };
    }
    ...
    if (error) {
      ...
      return {
        ok: false,
        error: error.message || "Error en la API de Resend",
      };
    }
